...
// Return the position in the read_buffer where the first record starts
// --------------------------------------------------------------------------------------------
inline int32 get_first_record(uchar *read_buffer, int32 start)
{
   	int32 position = 0;
	char checkChar = '@';
	...
	while(true)
	{
	  if (read_buffer[start] == checkChar)
	  {
	    if(checkChar == '@')
	    {
	      position = start;
...
// Return the position in the read_buffer where the last record ends
// --------------------------------------------------------------------------------------------
inline int32 get_last_record(uchar *read_buffer, int32 end)
{
   	int32 count_new_line = 0;

   	// Start localizing the last complete record
   	// before the last 5 lines in read_buffer
	while(true)
	{
		if (read_buffer[end--] == '\n')
			++count_new_line;
		...
		if (count_new_line == 5)
			return get_first_record(read_buffer, end + 2) - 1;	       
	}
}
