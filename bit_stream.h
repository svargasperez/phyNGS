...
public:
	...
	void SetIO_Buffer(const uchar *read_buffer, uint32 r_buff_size, uint32 start_pos = 0)
	{ 
		io_buffer_size = r_buff_size - start_pos;
		// io_buffer.resize(io_buffer_size);
		// std::copy(read_buffer+start_pos, read_buffer+r_buff_size, io_buffer.begin());
...
bool BitStream::GetByte(uint32 &byte)
{
	...
	{
		// printf("[%d] (byte) OVERFLOW: POS[%d] SIZE[%d]\n",IO_BUFFER_SIZE, io_buffer_pos, io_buffer_size);
		byte = 0;
		io_buffer_pos++;
		return true;
	}

	byte = io_buffer.at(io_buffer_pos++);
	file_pos++;
	return true;
}
...
bool BitStream::GetBytes(uchar *data, uint32 n_bytes)
{
	if (io_buffer_pos >= io_buffer_size)
	{
		// printf("[%d] (BYTES) OVERFLOW: POS[%d] SIZE[%d]\n",IO_BUFFER_SIZE, io_buffer_pos, io_buffer_size);
